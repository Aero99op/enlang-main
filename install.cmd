...
set "REPO_URL=https://raw.githubusercontent.com/Aero99op/enlang-main/main"
set "RELEASE_URL=https://github.com/Aero99op/enlang-main/releases/latest/download"
...
echo   Downloading latest production binaries from GitHub...
curl -fsSL "%REPO_URL%/enlangg.exe" -o "%INSTALL_DIR%\enlangg.exe" 2>nul
if %errorlevel% neq 0 (
    curl -fsSL "%RELEASE_URL%/enlangg.exe" -o "%INSTALL_DIR%\enlangg.exe"
)
curl -fsSL "%REPO_URL%/enlng.exe" -o "%INSTALL_DIR%\enlng.exe" 2>nul
if %errorlevel% neq 0 (
    curl -fsSL "%RELEASE_URL%/enlng.exe" -o "%INSTALL_DIR%\enlng.exe"
)
