...
"    m = re.match(r'^import\\s+python\\s+module\\s+\"([^\"]+)\"\\s+as\\s+([a-zA-Z0-9_]+)', trimmed)\n"
"    if m: return f'{indent}import {m.group(1)} as {m.group(2)}'\n"
"    m = re.match(r'^import\\s+python\\s+module\\s+\"([^\"]+)\"', trimmed)\n"
