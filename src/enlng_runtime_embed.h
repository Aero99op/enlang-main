...
static const char* ENLNG_EMBEDDED_RUNTIME =
    ...
    "    if (v.type == ENLNG_VAL_STRING && v.as.s) return (int64_t)atoll(v.as.s);\n"
    "    return 0;\n"
    "}\n"
    "\n"
    ...
    "            if (strcmp(key.as.s, \"at_last\") == 0) { int64_t cnt = enlng_count_of(c); return enlng_list_get(c, cnt > 0 ? cnt - 1 : 0); }\n"
    "            if (strcmp(key.as.s, \"at_first\") == 0) return enlng_list_get(c, 0);\n"
    "        }\n"
    ...
    "                if (idx < 0) idx = n + idx;\n"
    "                if (idx >= 0 && idx < n) buf[out_len++] = target.as.s[idx];\n"
    "            }\n"
    "            buf[out_len] = '\\0';\n"
    "            EnlngVal res; res.type = ENLNG_VAL_STRING; res.as.s = buf; return res;\n"
    "        }\n"
    ...
    "                int64_t idx = enlng_val_to_int(indices.as.l->items[i]);\n"
    "                if (idx < 0) idx = n + idx;\n"
    "                if (idx >= 0 && idx < n) enlng_list_push(copy, target.as.l->items[idx]);\n"
    "            }\n"
    "            return enlng_make_list(copy);\n"
    "        }\n"
    "        if (indices.type == ENLNG_VAL_INT) {\n"
    ...
    "            if (enlng_vals_equal(container.as.l->items[i], target)) count++;\n"
    "        }\n"
    "        return count;\n"
    "    }\n"
    "    return 0;\n"
    ...
    "    const char *s = container.as.s; const char *t = target.as.s;\n"
    "    const char *r = (repl.type == ENLNG_VAL_STRING && repl.as.s) ? repl.as.s : \"\";\n"
    "    size_t tlen = strlen(t); if (tlen == 0) return container;\n"
    "    size_t rlen = strlen(r); int count = 0;\n"
    "    const char *p = s;\n"
    "    while ((p = strstr(p, t)) != NULL) { count++; p += tlen; }\n"
    "    if (count == 0) return container;\n"
    "    size_t new_len = strlen(s) + count * (rlen > tlen ? (rlen - tlen) : 0) + 1;\n"
    "    char *buf = (char *)malloc(new_len + 64); buf[0] = '\\0';\n"
    "    p = s; const char *next;\n"
    "    while ((next = strstr(p, t)) != NULL) {\n"
    "        size_t chunk = (size_t)(next - p);\n"
    "        strncat(buf, p, chunk); strcat(buf, r); p = next + tlen;\n"
    "    }\n"
    "    strcat(buf, p); EnlngVal res = enlng_make_string(buf); free(buf); return res;\n"
    "}\n"
    "\n"
    ...
    "    if (container.type == ENLNG_VAL_LIST && container.as.l) {\n"
    "        for (int i = 0; i < container.as.l->count; i++) {\n"
    "            if (enlng_vals_equal(container.as.l->items[i], target)) return enlng_make_int(i);\n"
    "        }\n"
    ...
    "    char *buf = (char *)malloc(len + 1); strncpy(buf, s, len); buf[len] = '\\0';\n"
    "    EnlngVal res = enlng_make_string(buf); free(buf); return res;\n"
    "}\n"
    "\n"
    ...
    "    buf[strlen(v.as.s)] = '\\0'; EnlngVal res = enlng_make_string(buf); free(buf); return res;\n"
    "}\n"
    "\n"
    ...
    "    double total = 0; bool is_float = false;\n"
    "    for (int i = 0; i < list.as.l->count; i++) {\n"
    ...
    "    for (int i = 1; i < list.as.l->count; i++) { if (enlng_vals_gt(list.as.l->items[i], m)) m = list.as.l->items[i]; }\n"
    "    return m;\n"
    "}\n"
    "\n"
    ...
    "    if (list.type != ENLNG_VAL_LIST || !list.as.l || list.as.l->count == 0) return enlng_make_null();\n"
    "    EnlngVal m = list.as.l->items[0];\n"
    "    for (int i = 1; i < list.as.l->count; i++) { if (enlng_vals_lt(list.as.l->items[i], m)) m = list.as.l->items[i]; }\n"
    "    return m;\n"
    "}\n"
    "\n"
    ...
    "        if (strcmp(p, \"at_first\") == 0) return 0;\n"
    "        if (strcmp(p, \"at_last\") == 0) return is_add ? slen : (slen > 0 ? slen - 1 : 0);\n"
    ...
    "            return val;\n"
    "        }\n"
    "        if (is_add && val >= slen) return slen;\n"
    "        if (!is_add && val >= slen) return slen > 0 ? slen - 1 : 0;\n"
    "        return val;\n"
    "    }\n"
    ...
    "    EnlngVal item = arg1; EnlngVal target = arg2;\n"
    "    if (arg1.type == ENLNG_VAL_STRING && arg2.type == ENLNG_VAL_STRING && arg1.as.s && arg2.as.s) {\n"
    "        size_t l1 = strlen(arg1.as.s); size_t l2 = strlen(arg2.as.s);\n"
    "        if (l1 == 1 && l2 > 1) { item = arg1; target = arg2; }\n"
    "        else if (l1 > 1 && l2 == 1) { target = arg1; item = arg2; }\n"
    "        else if (l1 > l2) { target = arg1; item = arg2; }\n"
    "        else if (l1 < l2) {\n"
    "            int64_t pval = (pos.type == ENLNG_VAL_INT) ? pos.as.i : (pos.type == ENLNG_VAL_STRING && pos.as.s ? atoll(pos.as.s) : 0);\n"
    "            if (pval <= (int64_t)l1) { target = arg1; item = arg2; }\n"
    "            else { item = arg1; target = arg2; }\n"
    "        }\n"
    "    }\n"
    "    if (target.type != ENLNG_VAL_STRING || !target.as.s) return target;\n"
    "    const char* s = target.as.s;\n"
    "    size_t slen = strlen(s);\n"
    "    char temp[64]; const char* to_insert = \"\";\n"
    "    if (item.type == ENLNG_VAL_STRING && item.as.s) to_insert = item.as.s;\n"
    "    else if (item.type == ENLNG_VAL_INT) { snprintf(temp, sizeof(temp), \"%lld\", (long long)item.as.i); to_insert = temp; }\n"
    ...
    "    if (str.type != ENLNG_VAL_STRING || !str.as.s) return str;\n"
    "    const char* s = str.as.s;\n"
    "    size_t slen = strlen(s);\n"
    "    if (slen == 0) return str;\n"
    "    int64_t idx = enlng_parse_position(pos, (int64_t)slen, false);\n"
    "    if (idx < 0 || idx >= (int64_t)slen) return str;\n"
    ...
    "    if (str.type != ENLNG_VAL_STRING || !str.as.s) return str;\n"
    "    if (pos.type == ENLNG_VAL_INT) return enlng_builtin_string_remove_at(str, pos);\n"
    "    if (pos.type == ENLNG_VAL_STRING && pos.as.s) {\n"
    "        const char* p = pos.as.s;\n"
    "        if (strcmp(p, \"at_first\") == 0 || strcmp(p, \"at_last\") == 0) return enlng_builtin_string_remove_at(str, pos);\n"
    "        bool is_num = true; int i = 0;\n"
    "        if (p[0] == '-' && p[1] != '\\0') i = 1;\n"
    "        if (p[i] == '\\0') is_num = false;\n"
    "        for (; p[i] != '\\0'; i++) { if (p[i] < '0' || p[i] > '9') { is_num = false; break; } }\n"
    "        if (is_num) return enlng_builtin_string_remove_at(str, pos);\n"
    "        char* match_ptr = strstr(str.as.s, pos.as.s);\n"
    "        if (!match_ptr) return str;\n"
    ...
    "    EnlngVal new_val = arg1; EnlngVal target = arg2;\n"
    "    if (arg1.type == ENLNG_VAL_STRING && arg2.type == ENLNG_VAL_STRING && arg1.as.s && arg2.as.s) {\n"
    "        size_t l1 = strlen(arg1.as.s); size_t l2 = strlen(arg2.as.s);\n"
    "        if (l1 == 1 && l2 > 1) { new_val = arg1; target = arg2; }\n"
    "        else if (l1 > 1 && l2 == 1) { target = arg1; new_val = arg2; }\n"
    "        else if (l1 > l2) { target = arg1; new_val = arg2; }\n"
    "        else if (l1 < l2) {\n"
    "            int64_t pval = (pos.type == ENLNG_VAL_INT) ? pos.as.i : (pos.type == ENLNG_VAL_STRING && pos.as.s ? atoll(pos.as.s) : 0);\n"
    "            if (pval < (int64_t)l1) { target = arg1; new_val = arg2; }\n"
    "            else { new_val = arg1; target = arg2; }\n"
    "        }\n"
    "    }\n"
    "    if (target.type != ENLNG_VAL_STRING || !target.as.s) return target;\n"
    "    const char* s = target.as.s;\n"
    ...
    "    char temp[64]; const char* rep = \"\";\n"
    "    if (new_val.type == ENLNG_VAL_STRING && new_val.as.s) rep = new_val.as.s;\n"
    "    else if (new_val.type == ENLNG_VAL_INT) { snprintf(temp, sizeof(temp), \"%lld\", (long long)new_val.as.i); rep = temp; }\n"
