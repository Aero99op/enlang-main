...
static const char* ENLNG_EMBEDDED_RUNTIME =
    ...
    "    if (str.type != ENLNG_VAL_STRING || !str.as.s) return str;\n"
    "    if (target.type == ENLNG_VAL_INT) return enlng_builtin_string_remove_at(str, target);\n"
    "    if (target.type == ENLNG_VAL_STRING && target.as.s) {\n"
    ...
    "    char temp[64];\n"
    "    const char* c = \"\";\n"
    "    if (new_char.type == ENLNG_VAL_STRING && new_char.as.s) c = new_char.as.s;\n"
    "    else if (new_char.type == ENLNG_VAL_INT) { snprintf(temp, sizeof(temp), \"%lld\", (long long)new_char.as.i); c = temp; }\n"
