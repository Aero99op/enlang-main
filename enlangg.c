...
"def string_add(s, item, pos):\n"
"    s = str(s); item = str(item)\n"
"    try: pos = int(pos)\n"
"    except: pos = len(s)\n"
"    if pos < 0: pos = len(s) + pos\n"
"    pos = max(0, min(len(s), pos))\n"
"    return s[:pos] + item + s[pos:]\n"
"def string_insert(s, item, pos): return string_add(s, item, pos)\n"
"def string_remove_at(s, pos):\n"
"    s = str(s)\n"
"    try: pos = int(pos)\n"
"    except: return s\n"
"    if pos < 0: pos = len(s) + pos\n"
"    if 0 <= pos < len(s): return s[:pos] + s[pos+1:]\n"
"    return s\n"
"def string_remove(s, target):\n"
"    s = str(s)\n"
"    if isinstance(target, int):\n"
"        return string_remove_at(s, target)\n"
"    try:\n"
"        pos = int(target)\n"
"        return string_remove_at(s, pos)\n"
"    except:\n"
"        t = str(target); idx = s.find(t)\n"
"        if idx != -1: return s[:idx] + s[idx+len(t):]\n"
"        return s\n"
"def string_set_at(s, pos, new_char):\n"
"    s = str(s); nc = str(new_char)\n"
"    try: pos = int(pos)\n"
"    except: return s\n"
"    if pos < 0: pos = len(s) + pos\n"
"    if 0 <= pos < len(s): return s[:pos] + nc + s[pos+1:]\n"
"    return s\n"
"\n"
"# --- Built-in File I/O ---\n"
"def read_file(path):\n"
